...
@section('title', translate('product_price_inquiry_List'))
    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{ translate('product_price_inquiry_List') }}
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="searchValue" class="form-control"
                                            placeholder="{{ translate('search_by_contact_info') }}" aria-label="{{ translate('search_by_brand_name') }}" value="{{ request('searchValue') }}" required>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start">
                                ...
                                <tr>
                                    ...
                                    <th class="text-start">{{ translate('descriptions') }}</th>
                                    <th class="text-center">{{ translate('contact_info') }}</th>
                                    <th class="text-center">{{ translate('company') }}</th>
                                    <th class="text-center">{{ translate('dealer') }}</th>
                                    <th class="text-center">{{ translate('similar_info') }}</th>
                                ...
                                @foreach($products as $key => $brand)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.products.view',['addedBy'=>($brand->product->added_by=='seller'?'vendor' : 'in-house'),'id'=>$brand->product['id']]) }}"
                                        ...
                                        <td class="">
                                            <div>
                                                   <span class="d-block mb-1">
                                                    {{ translate('name')}} : {{ $brand['name'] }}
                                                   </span>
                                                   <span class="d-block mb-1">
                                                    {{ translate('email')}} : {{ $brand['email']}}
                                                   </span>
                                                   <span class="d-block mb-1">
                                                    {{ translate('phone')}} : {{ $brand['phone']}}
                                                   </span>
                                            </div>
                                        </td>
                                        <td class="text-center">{{ $brand['company'] }}</td>
                                        <td class="text-center">{{ $brand['is_dealer'] == 1 ? 'I am a dealer' : '' }}</td>
                                        <td class="text-center">{{ $brand['similar_info'] == 1 ? 'provide information of similar products' : '' }}</td>
                                        <td>
                                            <form action="{{route('admin.products.inquiry.price-status-update') }}" method="post" id="inquiry-status{{$brand['id']}}-form">
                                                @csrf
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}"
                                                    href="{{ route('admin.products.view',['addedBy'=>($brand->product->added_by=='seller'?'vendor' : 'in-house'),'id'=>$brand->product['id']]) }}">
                                            ...
                                            <form action="{{route('admin.products.inquiry.price-delete',[$brand['id']])}}"
...
<script>
  ...
  location.href = "{{ route('admin.products.inquiry.request-list')}}" +'?status=' + status;
    }
