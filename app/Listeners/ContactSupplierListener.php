...
class ContactSupplierListener
{
     ...
     * Handle the event.
     */
    public function handle(ContactSupplierEvent $event): void
    {
        if ($event->email) {
            $this->sendMail($event);
        }
    ...
    private function sendMail(ContactSupplierEvent $event): void
    {
        $email = $event->email;
        $data = $event->data;
        try {
    ...
    private function sendNotification(ContactSupplierEvent $event): void
    {
